 ...
 * image select Counter
 * add spinner for bonus
...
// image counter only selected image
const selectImage = document.getElementById('selected-imgCounter');
const selectImageCount = () => {
  selectImage.innerText = sliders.length;
}
...
const createSlider = () => {
  ...
  // creat slider previous next area
...
//Javascript key event search
document.getElementById('search').addEventListener('keypress', function (event) {
  if (event.key == 'Enter') {
    document.getElementById('search-btn').click();
    selectImage.innerText = 0;
...
sliderBtn.addEventListener('click', function () {
  ...
  if (duration >= 1000) {
    ...
    if (duration <= 1000) {
      ...
      durationError.innerText = `Minimum Duration Must be 1 Second or 1000ms`;
      durationError.style.color="red";
    }
  };
});

//keyboard enter button feature  slider 
